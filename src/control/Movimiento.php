    ...
    if ($objSesion->verificar_sesion_si_activa($id_sesion, $token)) {
        ...
        if (!empty($arr_Ambiente)) {
            ...
            for ($i = 0; $i < count($arr_Ambiente); $i++) {
                ...
                <a type="button" class="btn btn-primary waves-effect waves-light" href="'.BASE_URL.'imprimir-movimiento/'.$arr_Ambiente[$i]->id.'"><i class="fa fa-print"></i></a>
                ';
...
if ($tipo == "buscar_movimiento_id") {
    ...
    if ($objSesion->verificar_sesion_si_activa($id_sesion, $token)) {
        ...
        $arrIntitucion = $objInstitucion->buscarInstitucionById($arrMovimiento->id_ies);
    }
    echo json_encode($arrAmbOrigen);
    echo json_encode('<br>');
    echo json_encode($arrAmbDestino);
    echo json_encode('<br>');
    echo json_encode($arrIntitucion);
    echo json_encode('<br>');
